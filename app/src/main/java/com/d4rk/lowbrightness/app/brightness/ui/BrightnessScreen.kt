    ...
    LazyColumn(
        ...
        item {
            HomeItem(
                ...
                // TODO: Remove and add te settings in their layer
...
private fun HomeItem(imageVector : ImageVector , text : String , onClick : () -> Unit) {
    Card(
        modifier = Modifier.padding(vertical = 10.dp) , shape = RoundedCornerShape(percent = 100)
    ) {
        Row(modifier = Modifier
            ...
            Icon(
                modifier = Modifier.size(30.dp) , imageVector = imageVector , contentDescription = null
            )
            Text(
                modifier = Modifier.padding(start = 25.dp) , text = text , style = MaterialTheme.typography.titleLarge , maxLines = 1
            )
        }
    }
}
...
private fun IntensityCard() {
    ...
    Card(
        ...
        Column { // Removed padding from here
            // Header
            Box(
                modifier = Modifier
                        .fillMaxWidth()
                ...
                Text(
                    text = stringResource(id = R.string.intensity) , fontWeight = FontWeight.Bold , style = MaterialTheme.typography.titleLarge , // Or another appropriate style
                    color = MaterialTheme.colorScheme.onPrimary // Text color for on primary background
                )
            }
            // Content
            Column(modifier = Modifier.padding(16.dp)) { // Added padding here for content
                Text(
                    text = stringResource(id = R.string.summary_brightness) , style = MaterialTheme.typography.bodyMedium , modifier = Modifier.padding(top = 8.dp)
                )
                Slider(
                    modifier = Modifier.padding(top = 8.dp) , value = alpha , onValueChange = {
                        alpha = it
                        screenAlpha = it
...
private fun ColorCard() {
    ...
    Card(
        ...
        Column {
            ...
            Box(
                ...
                Text(
                    text = stringResource(id = R.string.customize_overlay_color) , fontWeight = FontWeight.Bold , style = MaterialTheme.typography.titleLarge , color = MaterialTheme.colorScheme.onPrimary
                )
            }
            // Content
            Column(modifier = Modifier.padding(16.dp)) {
                Row(
                    ...
                    Text(
                        ...
                        text = stringResource(id = R.string.summary_filter) , style = MaterialTheme.typography.bodyMedium
                    )
                    IconButton(onClick = { showDialog = true }) {
                        Box(
                            modifier = Modifier
                                    .size(48.dp)
                                    .background(
                                        color = Color(calculatedColor) , shape = RoundedCornerShape(percent = 50)
                                    )
                        )
                    }
                }
    ...
    if (showDialog) {
        ColorDialog(initColor = Color(screenColor) , onDismissRequest = { showDialog = false } , onColorSelected = { color ->
...
private fun BottomImage() {
    Card(
        ...
        Image(
            painter = painterResource(id = R.drawable.il_brightness) , contentDescription = null , modifier = Modifier.fillMaxSize() , contentScale = ContentScale.Crop
        )
    }
}